...
export class Sketch2018093003 extends Sketch {
  ...
  private u: CellArray = new CellArray([...Array(this.SIZE * this.SIZE)].map(() => 1), this.SIZE);
  private v: CellArray = new CellArray([...Array(this.SIZE * this.SIZE)].map(() => 0), this.SIZE);
  ...
  private initializeDensities(): void {
    ...
    this.u = this.u.map((cell, row, col) => {
      if (borderBottom <= row && row <= borderTop && borderBottom <= col && col <= borderTop) {
  ...
  private updateUv(): void {
    const laplacianU = this.u.roll("up")
      .add(this.u.roll("down"))
      .add(this.u.roll("left"))
      .add(this.u.roll("right"))
      .sub(this.u.mul(4))
      .div(this.dx * this.dx);
    const laplacianV = this.v.roll("up")
